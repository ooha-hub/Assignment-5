...
\end{align}
\begin{align}
\implies \vec{c}=\myvec{0\\0},\vec{f}=\vec{-4}
\\
\implies \vec{r}=\sqrt{\vec{c}^T\vec{x}-\vec{f}}=\sqrt{\vec{4}}
\\
\implies r &=2 \label{2.0.5}
\end{align}
From equation \eqref{2.0.5}, the point at which circle touches x-axis is \myvec{2\\0}.
\\
The area bounded by the circle and the lines ,$\vec{x}=\vec{0},\vec{x}=\vec{2}$,in the first quadrant is represented as shaded region in the plot.
\\ 
